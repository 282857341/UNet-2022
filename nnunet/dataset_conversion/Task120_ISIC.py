...
if __name__ == '__main__':
    ...
    base = '/home/xychen/jsguo'
    # this folder should have the training and testing subfolders

    # now start the conversion to nnU-Net:
    task_name = 'Task018_isic2016_right'
    ...
    # labels
    images_dir_tr= join(base, 'isic2016', 'jpg','Image')
    labels_dir_tr = join(base, 'isic2016', 'jpg','Label')
    ...
    images_dir_ts = glob.glob(join(base ,'ph2','bmp','Image','*.bmp'))
    labels_dir_ts = glob.glob(join(base ,'ph2','bmp','Label','*.bmp'))
    for ts in labels_dir_ts:
        unique_name = ts.split('/')[-1][:-4]
        output_seg_file = join(target_labelsTs, unique_name)
        convert_2d_image_to_nifti(ts, output_seg_file, is_seg=True,
                                  transform=lambda x: (x == 255).astype(int))
    for ts in images_dir_ts:
        unique_name = ts.split('/')[-1][:-4]
        output_seg_file = join(target_imagesTs, unique_name)
        convert_2d_image_to_nifti(ts, output_seg_file, is_seg=False)
    
    # finally we can call the utility for generating a dataset.json
    generate_dataset_json(join(target_base, 'dataset.json'), target_imagesTr, target_imagesTs, ('Red', 'Green', 'Blue'),
                          labels={1: 'lesion'}, dataset_name=task_name, license='hands off!')
