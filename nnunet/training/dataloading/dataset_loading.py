...
class DataLoader3D(SlimDataLoaderBase):
    ...
    def generate_train_batch(self):
        ...
        for j, i in enumerate(selected_keys):
            ...
            #case_all_data contain data and seg
            ...
            #exit()
            #force_fg=True
            #if not self.train and self.train!=None:
            #    force_fg=True
            ...
            if not force_fg:
                ...
                #else:
                #    foreground_classes = np.array(
                #    [i for i in properties['class_locations'].keys() if len(properties['class_locations'][i]) != 0 and i not in [5,9,10,12,13]])
...
class DataLoader2D(SlimDataLoaderBase):
    ...
    def generate_train_batch(self):
        ...
        for j, i in enumerate(selected_keys):
            ...
            #print(np.unique(case_all_data[-1]))
            ...
            if not force_fg:
                ...
                if len(foreground_classes) == 0:
                    selected_class = None
                    random_slice = np.random.choice(case_all_data.shape[1])
                    #print(properties['class_locations'][1])
                    #exit()
                    #print('case does not contain any foreground classes', i)
                else:
                    selected_class = np.random.choice(foreground_classes)
...
class DataLoader2D_unet2022(SlimDataLoaderBase):
    def __init__(self, data, num_threads_in_mt=12, batch_size=4,patch_size=(512,512),final_patch_size=None):
        # This initializes self._data, self.batch_size and self.number_of_threads_in_multithreaded
        super(DataLoader2D_unet2022, self).__init__(data, batch_size, num_threads_in_mt)
    ...
    def generate_train_batch(self):
        ...
        num_color=self._data[0].shape[0]-1
        data_shape = (self.batch_size, num_color, *self.patch_size)
        ...
        if idx < len(self._data):
            ...
            batch=self._data[idx: min(len(self._data),idx+self.batch_size)]
            for idx,case_all_data in enumerate(batch):
                case_all_data=case_all_data.squeeze(1)
                ...
                case_all_data_crop = case_all_data[:, valid_bbox_x_lb:valid_bbox_x_ub,
                                valid_bbox_y_lb:valid_bbox_y_ub]

                case_all_data_donly = np.pad(case_all_data_crop[:-1], ((0, 0),
                                                                  (-min(0, bbox_x_lb), max(bbox_x_ub - shape[0], 0)),
                                                                  (-min(0, bbox_y_lb), max(bbox_y_ub - shape[1], 0))),
                                             'constant')

                case_all_data_segonly = np.pad(case_all_data_crop[-1:], ((0, 0),
                                                                    (-min(0, bbox_x_lb), max(bbox_x_ub - shape[0], 0)),
                                                                    (-min(0, bbox_y_lb), max(bbox_y_ub - shape[1], 0))),
                                               'constant', **{'constant_values': -1})
             
                data[idx]=case_all_data_donly
                seg[idx]=case_all_data_segonly
